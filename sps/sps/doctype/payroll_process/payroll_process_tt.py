...
class PayrollProcess(Document):
	...
	def create_payout(self,employee=None, wage_rule=None):
		filters = [['docstatus', '<', 2],['period', '=', self.period]]
		if employee:
		...
		if pp_list:
			...
			#frappe.enqueue(create_salary_slips_and_payout, timeout=600, pp_list=pp_list, args=args)
		return pp_list
...
def create_salary_slips_and_payout(pp_list, args, publish_progress=True):
	...
	for pp_row in pp_list:
		...
		print"############## EMPpppppppppp ###########",pp_row["emp_id"]
		args.update({
			"doctype": "Salary Slip",
		...
		if publish_progress:
			frappe.publish_progress(count*100/len(set(pp_list)),itle = _("Creating Salary Slips..."))